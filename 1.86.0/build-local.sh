...
xtest() {
	args="--verbose --config ./config.toml --jobs $(/sbin/sysctl -n hw.ncpu) --target ${TARGET} --skip src/tools/linkchecker --skip src/tools/tidy"
	if [ -x /usr/local/bin/gdb ]; then
		args="${args} --set=build.gdb=/usr/local/bin/gdb"
	fi
	(cd $DEST/rustc-$RUST_VERSION-src && TMPDIR=/tmp ${PYTHON_BIN} x.py test ${args})
}
...
mkdir -p "${STAMP_DIR}"
info
RUN_LOCAL extract prepatch fixup-vendor config xbuild xtest xdist inst 2>&1
